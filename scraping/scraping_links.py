    ...
    try:
        if img_src_url.startswith('http'):
            ...
            if image_content:
                ...
                file_path = Path(images_dir, f"fish_http_{idx}.jpg")
                # file_path = Path(images_dir, f"{hashlib.sha1(img_content).hexdigest()[:10]}.jpg")
                image.save(file_path, "JPEG", optimize=True, quality=80)  
            ...
            if img_content:
                ...
                img_name = f"fish_b64_{idx}.jpg"
                # img_name = f"{hashlib.sha1(img_content).hexdigest()[:10]}.jpg"
                img_path = os.path.join(images_dir, img_name)
                img.save(img_path, "JPEG", optimize=True, quality=85)
...
if __name__ == "__main__":
    ...
    results = fetch_image_urls(150) # How many images you want?
